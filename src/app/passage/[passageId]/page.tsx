          ...
          <Reveal>
            <Card>
              <CardContent className="p-7">
                ...
                <p className="mt-4 whitespace-pre-wrap text-lg leading-9 text-slate-800">{passage.text}</p>
              </CardContent>
            </Card>
          </Reveal>
              ...
              <CardContent className="p-7">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <h2 className="display-font mt-2 text-2xl font-bold text-slate-950">الأسئلة المخزنة يدويًا</h2>
                  </div>
                  <Badge className="bg-slate-100 text-slate-700">{passage.questions.length} أسئلة</Badge>
                </div>

                <div className="mt-6 grid gap-4">
                  {passage.questions.map((question, index) => (
                    <div key={question.id} className="rounded-[1.7rem] border border-slate-200 bg-white p-5 shadow-sm">
                      <div className="display-font text-lg font-bold text-slate-950">سؤال {question.order || index + 1}</div>
                      ...
                      <div className="mt-4 grid gap-3">
                        {question.options.map((option) => (
                          ...
                          >
                            ...
                            <div className="mt-2">{question.explanations[option] || "لا يوجد شرح لهذا الخيار حاليًا."}</div>
                          </div>
                        ))}
                      </div>
                    </div>
